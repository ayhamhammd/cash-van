...
```ts
OfferType = 'PAYMENT_METHOD_DISCOUNT'   // more to come
...
1. Re-evaluates active offers against the cart (same engine as `/offers/evaluate`), reading the **payment method from `payments[0].paymentType`** (defaults CASH).
2. **Applies** the result to the voucher: per-line discounts → each line's `discountValue` (and the invoice discount → header discount / free lines appended — unused by current types).
...
Migration: `src/database/migrations/1718900000000-AddOffers.ts` (generic jsonb columns — new types need no migration). Demo payment-method offers are seeded by `src/database/seeds/run.ts`.
